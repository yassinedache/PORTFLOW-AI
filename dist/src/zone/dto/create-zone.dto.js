var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { IsString, IsUUID, IsOptional, IsInt, Min, IsBoolean, IsIn, } from 'class-validator';
import { ApiProperty, ApiPropertyOptional, PartialType } from '@nestjs/swagger';
export class CreateZoneDto {
    name;
    terminalId;
    type;
    maxTrucks;
}
__decorate([
    ApiProperty({ description: 'Zone name', example: 'Hazmat Area A' }),
    IsString(),
    __metadata("design:type", String)
], CreateZoneDto.prototype, "name", void 0);
__decorate([
    ApiProperty({ description: 'Terminal ID' }),
    IsUUID(),
    __metadata("design:type", String)
], CreateZoneDto.prototype, "terminalId", void 0);
__decorate([
    ApiPropertyOptional({
        description: 'Zone type',
        enum: ['GENERAL', 'HAZMAT', 'REEFER', 'OVERSIZED'],
        default: 'GENERAL',
    }),
    IsOptional(),
    IsString(),
    IsIn(['GENERAL', 'HAZMAT', 'REEFER', 'OVERSIZED']),
    __metadata("design:type", String)
], CreateZoneDto.prototype, "type", void 0);
__decorate([
    ApiPropertyOptional({ description: 'Maximum trucks in zone', default: 50 }),
    IsOptional(),
    IsInt(),
    Min(1),
    __metadata("design:type", Number)
], CreateZoneDto.prototype, "maxTrucks", void 0);
export class UpdateZoneDto extends PartialType(CreateZoneDto) {
    isActive;
}
__decorate([
    ApiPropertyOptional({ description: 'Whether zone is active' }),
    IsOptional(),
    IsBoolean(),
    __metadata("design:type", Boolean)
], UpdateZoneDto.prototype, "isActive", void 0);
//# sourceMappingURL=create-zone.dto.js.map